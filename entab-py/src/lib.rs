...
use entab_base::readers::{RecordReader, get_reader};
...
fn py_from_value(value: Value, py: Python) -> PyResult<Py<PyAny>> {
    Ok(match value {
        Value::Null => py.None(),
        Value::Boolean(b) => PyBool::new(py, b).to_owned().into(),
        Value::Datetime(d) => {
            // NB: For files without timezone data (and all NaiveDateTime?),
            // .format("%+") panics. So timezone is omitted
            d.format("%Y-%m-%dT%H:%M:%S%.f")
                .to_string()
                .into_pyobject(py)
                .unwrap()
                .unbind()
            ...
            //            pyo3::types::PyDateTime::from_timestamp(py, timestamp,
            //            None)?.into_pyobject(py)
        }
        Value::Float(v) => PyFloat::new(py, v).into(),
        Value::Integer(v) => PyInt::new(py, v).into(),
...
impl Reader {
    ...
    #[getter]
    pub fn get_metadata(&self, py: Python) -> PyResult<Py<PyAny>> {
    ...
    fn __iter__(slf: PyRefMut<Self>, py: Python) -> PyResult<Py<PyAny>> {
        let val = slf.into_pyobject(py).unwrap().unbind().into_any();
        Ok(val)
    }
...
mod tests {
    ...
    fn test_reader_creation() -> PyResult<()> {
        ...
        Python::attach(|py| {
            ...
            let test_data: Bound<PyAny> = b">test\nACGT".into_pyobject(py).unwrap();
            let reader = Reader::new(Some(&test_data), None, None, py)?;
            assert_eq!(&reader.parser, "fasta");

            // metadata are available
            let metadata = reader.get_metadata(py)?;
            assert!(metadata.bind(py).cast::<PyDict>().is_ok());
    ...
    fn test_reader_in_python() -> PyResult<()> {
        ...
        Python::attach(|py| {
            ...
            entab(&module)?;
            let locals = pyo3::types::IntoPyDict::into_py_dict([("entab", &module)], py)?;
